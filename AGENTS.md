...
**Control flow belongs in a coordinator.** If a change adds ordering, branching, a bound, a retry, or a resume point to a workflow document in prose, it belongs in `scripts/` instead. Prose cannot resume and cannot prove a bound held. Two shapes of this are now checked, both in the imperative only: a repeat verb carrying a numeric bound, and a transfer of control naming a phase or step. Describing what a coordinator does stays legitimate and unflagged, because the discriminator is grammatical mood rather than vocabulary. The rest of the rule, ordering and conditional branching in general, remains judgement: a vocabulary scan for it matched 28 correct sentences in these five workflows alone, so no pattern for it could fire without punishing accurate prose.
