...
def within(posix, prefixes):
    """Whether a path lies at or under one of these directory prefixes.

    A string prefix is not a path prefix, and the difference is not academic:
    `str.startswith("references/upstream")` also matches
    `references/upstream-notes.md`, which is not vendored and is not inside
    that directory. Anyone writing new guidance under a name beginning those
    characters would buy a silent exemption from the rule and have no reason
    to know it. The prefix has to be followed by a separator or be the whole
    path.

    state.ignored asks the same question for the absorption snapshot, but
    importing it here would make this module depend on the schema layer that
    `cli.py` and `names.py` are deliberately free of, and a skill author is
    meant to be able to copy this file.
    """
...
def is_core_guidance(relative):
    ...
    if within(posix, NEUTRAL_EXEMPT):
        return False
...
def self_check():
    ...
    # The carve-out is a directory, not a spelling. These two are ordinary
    # guidance that merely begins with the same characters, and a string
    # prefix exempted both from the rule this repository most exists to state.
    assert is_core_guidance(Path("references/upstream.md")), "a string prefix exempted a file, not a directory"
    assert is_core_guidance(Path("references/upstream-notes.md")), "a string prefix exempted a sibling name"
    assert within("references/upstream", NEUTRAL_EXEMPT)
    assert within("references/upstream/a.md", NEUTRAL_EXEMPT)
    assert not within("references/upstream-notes.md", NEUTRAL_EXEMPT)
